"""final idempotent categories/transactions migration

Revision ID: 2025d0a8c587
Revises: 14022d4cc24d
Create Date: 2026-02-15 13:23:04.128472
...
def upgrade() -> None:
    ...
    # 1. Categories table – create only if not exists
    if not inspector.has_table('categories'):
        ...
        # Seed defaults (idempotent – only insert if not present)
        defaults = [
    ...
    # 3. Migrate string category to category_id (safe – only NULLs)
    op.execute("""
        UPDATE budget_items 
        SET category_id = (
    ...
    # 4. Fallback unmatched to "Other Income"/"Other Expense"
    op.execute("""
        UPDATE budget_items 
        SET category_id = (
            SELECT id FROM categories 
    ...
    # 5. Make category_id non-nullable
