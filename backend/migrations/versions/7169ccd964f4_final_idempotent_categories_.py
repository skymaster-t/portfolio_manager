"""final idempotent categories/transactions (WHERE NOT EXISTS safe)

Revision ID: 7169ccd964f4
Revises: f30249e71447
Create Date: 2026-02-15 17:11:22.764153
...
revision: str = '7169ccd964f4'
down_revision: Union[str, Sequence[str], None] = '344bcd042bf6'
...
def upgrade() -> None:
    ...
    # Seed defaults – safe WHERE NOT EXISTS (no ON CONFLICT, no constraint needed)
    defaults = [
    ...
    for name, cat_type in defaults:
        op.execute(text(f"""
            ...
            WHERE NOT EXISTS (
                SELECT 1 FROM categories WHERE user_id = 1 AND name = :name
            )
